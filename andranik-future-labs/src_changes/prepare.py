...
def runStages():
    ...
    for stage in stages:
        ...
        # Patch for Insiders 2026 and v143 toolset
        ...
        if 'msbuild' in commands:
            # Inject python patcher before msbuild for breakpad
            if stage['name'] == 'breakpad':
                ...
                # The original command is: msbuild -m dump_syms.vcxproj ...
                # We replace it with a command that stays in the root directory
                # Breakpad specific fixes for VS2026 and directory structure
                commands = commands.replace('cd src\\client\\windows', 'rem')
                commands = commands.replace('gyp --no-circular-check breakpad_client.gyp --format=ninja', 'cd src\\client\\windows && gyp --no-circular-check breakpad_client.gyp --format=ninja && cd ..\\..\\..')
                commands = commands.replace('cd tools\\windows\\dump_syms', 'rem')
                commands = commands.replace('gyp dump_syms.gyp --format=msvs', 'rem')
                commands = commands.replace('msbuild -m dump_syms.vcxproj /property:Configuration=Release /property:Platform="x64"', f'cd src\\tools\\windows\\dump_syms && gyp dump_syms.gyp --format=msvs && {patch_cmd} && {msbuild_path} /p:Configuration=Release /p:Platform=x64 /p:PlatformToolset=v143 /p:WindowsTargetPlatformVersion=10.0 /p:VCTargetsPath="{vc_targets}" dump_syms.vcxproj && cd ..\\..\\..')

            # Add toolset and targets path to all msbuild calls
